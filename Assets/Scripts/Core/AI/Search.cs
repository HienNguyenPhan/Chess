namespace Chess {
	...
	public class Search {
		...
		public void StartSearch () {
			...
			if (settings.useIterativeDeepening) {
				...
				for (int searchDepth = 1; searchDepth <= targetDepth; searchDepth++) {
					...
					if (!Move.SameMove(bestMoveThisIteration, Move.NullMove)) {
		...
		int SearchMoves (int depth, int plyFromRoot, int alpha, int beta, int numExtension = 0) {
			...
			Move searchThisMoveFirst = plyFromRoot == 0 ? bestMove : tt.GetStoredMove();
			moveOrdering.OrderMoves (board, moves, searchThisMoveFirst);
		...
		int QuiescenceSearch (int alpha, int beta) {
			...
			moveOrdering.OrderMoves (board, moves, tt.GetStoredMove());
			for (int i = 0; i < moves.Count; i++) {
